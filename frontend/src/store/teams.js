...
export const getAllTeamsThunk = () => async (dispatch) => {
  ...
  if (response.ok) {
    ...
    await dispatch(addAllTeams(teams));
  }
};
...
export const createNewTeamThunk = (team) => async (dispatch) => {
  ...
  if (response.ok) {
    ...
    await dispatch(addCurrentTeam(team));
    return null;
  } else {
...
export const getUserTeamsThunk = (id) => async (dispatch) => {
  ...
  if (response.ok) {
    const teams = await response.json();
    await dispatch(addUserTeams(teams.userTeams));
    return null;
  } else {
    const errorResponse = await response.json();
    return errorResponse.errors;
...
const teamsReducer = (state = initialState, action) => {
  switch (action.type) {
    case ADD_TEAMS: {
      ...
      newState.allTeams = normalize(action.payload.teams);
    ...
    case ADD_USER_TEAMS: {
      const newState = normalize(state);
      newState.userTeams = normalize(action.payload);
    ...
    case ADD_CURRENT_TEAM: {
      const newState = normalize(state);
      newState.currentTeam = normalize(action.payload.team);
      return newState;
    }
    default:
      return state;
